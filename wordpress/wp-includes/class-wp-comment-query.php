...
class WP_Comment_Query {
	...
	public function get_comments() {
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$this->meta_query_clauses = $this->meta_query->get_query( 'comment', $wpdb->comments, 'comment_ID', $this );
		}
	...
	protected function get_comment_ids() {
		...
		if ( isset( $this->query_vars['search'] ) && strlen( $this->query_vars['search'] ) ) {
			$search_sql = $this->get_search_query(
		...
		if ( ! empty( $this->query_vars['date_query'] ) && is_array( $this->query_vars['date_query'] ) ) {
			...
			// Strip leading 'AND'.
			$this->sql_clauses['where']['date_query'] = preg_replace( '/^\s*AND\s*/', '', $this->date_query->get_query() );
		}
	 ...
	 */
	protected function get_search_query( $search, $columns ) {
		global $wpdb;
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( $this->query_vars['meta_key'] === $orderby || 'meta_value' === $orderby ) {
			...
			if ( isset( $meta_query_clauses[ $orderby ] ) ) {
				...
				$parsed      = sprintf( 'CAST(%s.meta_value AS %s)', esc_query( $meta_clause['alias'] ), esc_sql( $meta_clause['cast'] ) );
			} else {
