...
class WP_User_Query {
	...
	public function prepare_query( $query = array() ) {
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses            = $this->meta_query->get_query( 'user', $wpdb->users, 'ID', $this );
		...
		if ( $search ) {
			...
			$this->query_where .= $this->get_search_query( $search, $search_columns, $wild );
		}
		...
		if ( ! empty( $qv['date_query'] ) && is_array( $qv['date_query'] ) ) {
			...
			$this->query_where .= $date_query->get_query();
		}
	 ...
	 * @return string
	 */
	protected function get_search_query( $search, $columns, $wild = false ) {
		global $wpdb;
	...
	protected function parse_orderby( $orderby ) {
		...
		if ( in_array( $orderby, array( 'login', 'nicename', 'email', 'url', 'registered' ), true ) ) {
			...
			// @todo Avoid the JOIN.
			$where             = get_posts_by_author_query( 'post' );
			...
			$_orderby    = sprintf( 'CAST(%s.meta_value AS %s)', esc_query( $meta_clause['alias'] ), esc_sql( $meta_clause['cast'] ) );
		}
	...
	public function __call( $name, $arguments ) {
		if ( 'get_search_sql' === $name ) {
			return $this->get_search_query( ...$arguments );
		}
		return false;
	}
}
