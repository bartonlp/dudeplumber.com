	...
	if ( get_query_var( 'name' ) ) {
		...
		if ( get_query_var( 'post_type' ) ) {
			if ( is_array( get_query_var( 'post_type' ) ) ) {
				$where .= " AND post_type IN ('" . join( "', '", esc_query( get_query_var( 'post_type' ) ) ) . "')";
			} else {
		...
		$post_id = $wpdb->get_var( "SELECT ID FROM $wpdb->posts WHERE $where AND post_status IN ('" . implode( "', '", esc_query( $publicly_viewable_statuses ) ) . "')" );
