...
class WP_Query {
	...
	/**
	 * Taxonomy query, as passed to get_tax_query().
	 *
	 * @since 3.1.0
	...
	public function get_posts() {
		...
		if ( ! empty( $q['date_query'] ) ) {
			...
			$where           .= $this->date_query->get_query();
		}
		...
		if ( ! $this->is_singular ) {
			...
			$clauses = $this->tax_query->get_query( $wpdb->posts, 'ID' );
		...
		if ( ! empty( $this->meta_query->queries ) ) {
			$clauses = $this->meta_query->get_query( 'post', $wpdb->posts, 'ID', $this );
			$join   .= $clauses['join'];
			$where  .= $clauses['where'];
		}
		...
		if ( 'any' === $post_type ) {
			...
			$post_type_where = " AND {$wpdb->posts}.post_type IN ('" . implode( "', '", esc_query( $post_type ) ) . "')";
